...
class FingerprintEnabledClient(A2AClient):
    ...
    def __init__(
        ...
        self.fingerprint_validator = fingerprint_validator or FingerprintValidator()
    ...
    def _sign_message(self, message: Message) -> Message:
        ...
        message_hash = self.fingerprint_generator.compute_message_hash(message_dict)
    ...
    async def send_task(self, payload: Dict[str, Any]) -> Any:
        ...
        if self.verify_incoming and response.result and response.result.status.message:
            try:
