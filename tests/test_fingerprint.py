...
from samples.python.common.types_fingerprint import Fingerprint, MessageVerification
from samples.python.common.utils.fingerprint_generator import FingerprintGenerator
from samples.python.common.utils.fingerprint_validator import FingerprintValidator
...
def test_fingerprint_verification(fingerprint_generator, fingerprint_validator, sample_fingerprint):
    """Test that fingerprints can be verified correctly."""
    # Verify a valid fingerprint
    with patch.object(fingerprint_validator, '_verify_signature', return_value=True):
        with patch.object(fingerprint_validator, '_verify_on_blockchain', return_value=True):
            assert fingerprint_validator.validate_fingerprint(sample_fingerprint) is True
    
    # Test with invalid signature
    with patch.object(fingerprint_validator, '_verify_signature', return_value=False):
    ...
    with patch.object(fingerprint_validator, '_verify_signature', return_value=True):
        with patch.object(fingerprint_validator, '_verify_on_blockchain', return_value=False):
...
def test_message_signing_and_verification(fingerprint_generator, fingerprint_validator, sample_fingerprint, sample_message):
    ...
    # Verify the message
    with patch.object(fingerprint_validator, 'validate_fingerprint', return_value=True):
        assert fingerprint_validator.verify_message(
...
def test_fingerprint_serialization(sample_fingerprint):
    ...
    assert deserialized.timestamp.isoformat() == sample_fingerprint.timestamp.isoformat()
    assert deserialized.validation_endpoint == sample_fingerprint.validation_endpoint