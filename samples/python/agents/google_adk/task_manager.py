...
def update_session_state(changes: dict[str, Any], session: Session, session_service: BaseSessionService):
...
class AgentWithTaskManager(ABC):
    ...
    async def update_credentials(self, state: str, credentials: dict[str, Any]):
        # Should always be there
        authn_state = self._authn_state[state]
        session_state = authn_state['session']
        session = get_session(
            self._runner.session_service,
            session_state['app_name'],
            session_state['user_id'],
            session_state['session_id'],
        )
        orig_request = authn_state['request']
        new_request = orig_request.copy(deep=True)
        retry_part = TextPart(text='Credentials have been provided to the agent, please retry.')

        # Add credentials to the session state
        ...
        # TODO: A real implemementation would likely have a message from the agent to the user acknowledging that the
        #       credentials have been received.
        if isinstance(orig_request, SendMessageRequest):
            new_request.id = str(uuid.uuid4())
            new_request.params.id = str(uuid.uuid4())
            new_request.params.message.contextId = authn_state['context_id']
            new_request.params.message.messageId = str(uuid.uuid4())
            new_request.params.message.parts = [retry_part]
            new_request.params.message.taskId = authn_state['task_id']

            # Retry the message/task
            response = await self._task_manager.on_send_message(new_request)
        else:
            new_request.params.id = str(uuid.uuid4())
            new_request.params.message.contextId = authn_state['context_id']
            new_request.params.message.messageId = str(uuid.uuid4())
            ...
            # Retry the message/task
            response = await self._task_manager.on_send_message_stream(new_request)

        # Remove the state
        del self._authn_state[state]
    ...
    def invoke(self, send_params, session_id) -> str:
        ...
        if send_params.message.metadata and 'oauth_redirect_uri' in send_params.message.metadata:
            update_session_state(
                {
                    'oauth_redirect_uri': send_params.message.metadata['oauth_redirect_uri'],
                },
                session,
                self._runner.session_service,
            )
        ...
        # If there is an A2A authentication error, return a corresponding A2AClientError
        if a2a_client_error:
            # Persist information in data to get back to the session storage
            self._authn_state[a2a_client_error.data['state']] = {
                'context_id': send_params.message.contextId,
                'session': {
                    'app_name': self._agent.name,
                    'session_id': session_id,
                    'user_id': self._user_id,
                },
                'task_id': send_params.message.taskId,
            }
...
class AgentTaskManager(InMemoryTaskManager):
            ...
            async for item in self.agent.stream(send_params, contextId):
                ...
                if not is_task_complete:
                    if item.get('auth_required'):
                        authn_state = self.agent._authn_state[item['state']]

                        authn_state['request'] = request
    ...
    async def _send(self, request: SendMessageRequest) -> SendMessageResponse:
        ...
        if task_state == TaskState.AUTH_REQUIRED:
            ...
            if state not in self.agent._authn_state:
                self.agent._authn_state[state] = {}

            authn_state = self.agent._authn_state[state]
