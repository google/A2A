...
class TaskManager(InMemoryTaskManager):
    ...
    async def on_send_message_stream(
        ...
        """A method to handle a message stream request.

        Args:
            request: The message streaming request containing the parameters.

        Returns:
            AsyncIterable[SendMessageStreamResponse] | JSONRPCResponse: The streaming response or error.
        """
        try:
            error = self._validate_request(request)
            if error:
    ...
    async def _run_streaming_message_agent(
        ...
        Yields:
            AsyncIterable[SendMessageStreamResponse]: The streaming response.
        """
        try:
