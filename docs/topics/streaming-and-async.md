...
- **Event Structure:** The server sends events over this stream. Each event's `data` field contains a JSON-RPC 2.0 Response object, specifically a [`SendTaskStreamingResponse`](../specification.md#721-sendtaskstreamingresponse-object). The `id` in this JSON-RPC response matches the `id` from the client's original `tasks/sendSubscribe` request.
- **Event Types (within `SendTaskStreamingResponse.result`):**
  ...
  - [`TaskArtifactUpdateEvent`](../specification.md#723-taskartifactupdateevent-object): Delivers new or updated [Artifacts](../specification.md#67-artifact-object) generated by the task. This is used to stream large files or data structures in chunks. The `Artifact` object itself contains fields like `index`, `append`, and `lastChunk` to help the client reassemble the complete artifact.
...
- [`tasks/sendSubscribe`](../specification.md#72-taskssendsubscribe)
...
- **Configuration:** The client provides a [`PushNotificationConfig`](../specification.md#68-pushnotificationconfig-object) to the server. This configuration can be supplied:
  ...
  - Separately, using the `tasks/pushNotification/set` RPC method for an existing task.
  ...
  - `authentication` (optional): An [`AuthenticationInfo`](../specification.md#69-authenticationinfo-object-for-push-notifications) object specifying how the A2A Server should authenticate itself *to the client's webhook URL*. The client (receiver of the webhook) defines these authentication requirements.
