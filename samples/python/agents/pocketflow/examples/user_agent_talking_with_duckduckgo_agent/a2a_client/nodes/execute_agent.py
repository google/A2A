...
class ExecuteAgentNode(AsyncNode):
        ...
        payloads_and_agents = [
            (
                construct_payload(
                    uuid4().hex,
                    session_id,
                    selected_agent['question_to_answer'],
                ),
    ...
    async def exec_async(
        ...
        logger.info(
            f'⏳ {colorama.Fore.YELLOW}Executing agents{colorama.Style.RESET_ALL}'
        )
        ...
        logger.info(
            f'✅ {colorama.Fore.GREEN}Finished executing agents{colorama.Style.RESET_ALL}'
        )
        logger.info(
            f'{colorama.Fore.GREEN}Responses{colorama.Style.RESET_ALL}: {responses}'
        )
        return list(zip(payloads_and_agents, responses, strict=False))
        ...
        for (payload, agent_card), response in exec_res:
            ...
            agent_contexts.append(
                AgentContext(
                    ...
                    agent_skills=[
                        skill.model_dump() for skill in agent_card.skills
                    ],
