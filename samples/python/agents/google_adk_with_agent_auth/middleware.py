...
class GoogleOAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Use the provided OAuth Redirect URI from metadata (if provided)
        try:
            data = await request.json()

            if isinstance(data, dict) and \
                (params := data.get("params")) and \
                (message := params.get("message")) and \
                (metadata := message.get("metadata")) and \
                (uri := metadata.get("oauth_redirect_uri")):
                    request.app.state.oauth_redirect_uri = uri
