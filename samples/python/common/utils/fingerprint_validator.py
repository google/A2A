...
class FingerprintValidator:
    ...
    def __init__(self, ethereum_url: Optional[str] = None, cache_ttl: int = 3600):
        ...
        """
        self.w3 = Web3(Web3.HTTPProvider(ethereum_url or "https://sepolia.infura.io/v3/"))
    ...
    def validate_fingerprint(
        ...
        try:
            ...
            signature_valid = self._verify_signature(fingerprint.hash, fingerprint.signature)
            if not signature_valid:
                self._cache_result(cache_key, False)
                raise FingerprintVerificationError("Invalid signature")
        except Exception as e:
            self._cache_result(cache_key, False)
            raise FingerprintVerificationError(f"Signature verification error: {str(e)}")
        ...
        if check_blockchain and fingerprint.blockchain_ref:
            try:
                ...
                if not blockchain_valid:
                    self._cache_result(cache_key, False)
                    raise FingerprintVerificationError("Blockchain verification failed")
            except Exception as e:
                self._cache_result(cache_key, False)
                raise FingerprintVerificationError(f"Blockchain verification error: {str(e)}")
        ...
        if check_endpoint and fingerprint.validation_endpoint:
            try:
                ...
                if not endpoint_valid:
                    self._cache_result(cache_key, False)
                    raise FingerprintVerificationError("Endpoint verification failed")
            except Exception as e:
                self._cache_result(cache_key, False)
                raise FingerprintVerificationError(f"Endpoint verification error: {str(e)}")
    ...
    def verify_message(
        ...
        try:
            ...
            if isinstance(message_content, dict):
                ...
                message_str = "&".join(f"{k}={v}" for k, v in sorted(message_content.items()))
